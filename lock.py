...
class Fullscreen_Window:
	...
	def show_idle(self):
		self.welcomeLabel = ttk.Label(self.tk, text="Veuillez Présenter\nVotre Badge")
	...
	def listen_rfid(self):
		...
		while True:
			...
			if cur.rowcount == 1:
				...
				isLoggedIn = True
			if isLoggedIn == False:
				self.welcomeLabel.config(text="ACCÈS REFUSÉ")
	...
	def validate(self):
		...
		if pinInput == userPin:
			self.PINresultLabel = ttk.Label(self.tk, text="Merci,\nAccès autorisé")
			...
			self.PINresultLabel = ttk.Label(self.tk, text="PIN Incorrect !\n")
			self.PINresultLabel.config(font='size, 20', justify='center', anchor='center')
